...
**Data Dimensions:** This dataset contains `r nrow(data)` rows and `r ncol(data)` columns.
...
* **`uid`**: Unique identifier for each homicide case, with `r length(unique(data$uid))` unique values in the dataset.
* **`reported_date`**: The date the homicide was reported, in the format YYYYMMDD. Ensuring consistent date format is essential for accurate time-based analysis.
* **`victim_last`** and **`victim_first`**: The last and first names of the victim, respectively. These fields allow for identification but are not typically used for statistical analysis.
* **`victim_race`**: The race of the victim, including categories such as `r toString(unique(data$victim_race))`.
* **`victim_age`**: Age of the victim at the time of the homicide, ranging from `r min(data$victim_age, na.rm = TRUE)` to `r max(data$victim_age, na.rm = TRUE)`. Filtering out unrealistic ages (e.g., below 0 or above 120) may be necessary for accurate analysis.
* **`victim_sex`**: The gender of the victim, including `r toString(unique(data$victim_sex))`.
* **`city`** and **`state`**: The location of the homicide. The dataset covers `r length(unique(data$city))` cities and `r length(unique(data$state))` states.
* **`lat`** and **`lon`**: Latitude and longitude coordinates for the location of each homicide, enabling geospatial analysis.
* **`disposition`**: The case status, indicating whether it was solved. Categories include: `r toString(unique(data$disposition))`. This variable is essential for analyzing the proportion of unsolved cases.
